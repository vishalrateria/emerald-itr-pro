...
def get_hardware_profile() -> str:
    ...
    override = SettingsManager.get("AI.hardware_profile", "auto")
...
def get_hardware_config(profile: str) -> dict:
    ...
    config = configs.get(profile, configs["eco"]).copy()
    ...
    if "AI" in settings:
        ...
        if "n_ctx" in ai_settings:
            config["n_ctx"] = ai_settings["n_ctx"]
        if "n_gpu_layers" in ai_settings:
            config["n_gpu_layers"] = ai_settings["n_gpu_layers"]
        if "n_threads" in ai_settings:
            config["n_threads"] = ai_settings["n_threads"]

    return config


def sanitize_pii(text: str) -> str:
    text = re.sub(r"\b[A-Z]{5}([0-9]{4})[A-Z]{1}\b", r"XXXXX\g<1>X", text)
    text = re.sub(r"\b\d{4}[\s-]?\d{4}[\s-]?(\d{4})\b", r"XXXX-XXXX-\g<1>", text)
    return text
