...
def remove_empty_nodes(data):
    if isinstance(data, dict):
        cleaned = {k: remove_empty_nodes(v) for k, v in data.items()}
        return {k: v for k, v in cleaned.items() if v not in [{}, [], "", None]}
    elif isinstance(data, list):
        cleaned = [remove_empty_nodes(item) for item in data]
...
class ITRMapperService:
    ...
    def map_to_itr(form_type: str, v: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            base = {
                "ITR": {
                    form_key: {
                        ...
                        "Schedule_TDS1": ITRMapperService._map_tds(
                            v, section_filter=["192"]
                        ),
                        "Schedule_TDS2": ITRMapperService._map_tds(
                            v, section_filter=["194"]
                        ),
            ...
            if form_key in ["ITR2", "ITR3", "ITR4"]:
                ...
                base["ITR"][form_key]["Schedule_BP"] = ITRMapperService._map_bp(
                    v, form_key
                )
            if form_key in ["ITR2", "ITR3"]:
                ...
                if ITRMapperService._txt(v, "has_foreign_assets", "0") in [
                    "1",
                    "yes",
                    "true",
                ]:
            ...
            if ITRMapperService._val(v, "tti") > 5000000 and form_key in [
                "ITR2",
                "ITR3",
                "ITR4",
            ]:
                base["ITR"][form_key]["Schedule_AL"] = (
                    ITRMapperService._map_schedule_al(v)
                )
    ...
    def _map_part_a(v: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "PersonalInfo": {
                ...
                "AssesseeName": {"FirstName": ITRMapperService._txt(v, "name")},
            ...
            "FilingStatus": {
                ...
                "ResidentialStatus": (
                    "RES"
                    if "non-resident"
                    not in ITRMapperService._txt(v, "filing_status").lower()
                    else "NRI"
                ),
            },
        }
    ...
    def _map_bank_accounts(v: Dict[str, Any]) -> Dict[str, Any]:
        accounts = []
        for i in range(5):
            acc_no = ITRMapperService._sanitize(
                ITRMapperService._txt(v, f"bank_{i}_acc")
            )
            if acc_no:
                accounts.append(
                    {
                        "BankAccountNo": acc_no,
                        "IFSCCode": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"bank_{i}_ifsc")
                        ),
                        "BankName": ITRMapperService._txt(v, f"bank_{i}_name"),
                        "UseForRefund": (
                            "Yes"
                            if str(ITRMapperService._txt(v, f"bank_{i}_refund")).lower()
                            in ["yes", "true", "1"]
                            else "No"
                        ),
                    }
    ...
    def _map_tds(v: Dict[str, Any], section_filter: List[str]) -> List[Dict[str, Any]]:
        ...
        for i in range(10):
            ...
            is_match = (
                any(sec in section for sec in section_filter)
                if section
                else ("192" in section_filter)
            )
            if amt > 0 and is_match:
                tds_list.append(
                    {
                        "TAN": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"tds_{i}_tan")
                        ),
        ...
        for i in range(10):
            ...
            if amt > 0:
                tcs_list.append(
                    {
                        "TAN": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"tcs_{i}_tan")
                        ),
    ...
    def _map_advance_tax(v: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(10):
            ...
            if amt > 0:
                tax_type = (
                    "100"
                    if "advance" in ITRMapperService._txt(v, f"tax_{i}_type").lower()
                    else "300"
                )
                adv_list.append(
                    {
                        "TaxType": tax_type,
                        "BSRCode": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"tax_{i}_bsr")
                        ),
                        "DateOfDeposit": ITRMapperService._txt(v, f"tax_{i}_date"),
                        "ChallanNo": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"tax_{i}_challan")
                        ),
                        "Amount": int(amt),
                    }
                )
    ...
    def _map_hp(v: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(2):
            ...
            if rent > 0 or ITRMapperService._txt(v, f"hp_{i}_type") == "self-occupied":
                hp_list.append(
                    {
                        "PropertyType": ITRMapperService._txt(
                            v, f"hp_{i}_type"
                        ).upper(),
                        "Address": {
                        ...
                        "TenantDetails": {
                            ...
                            "PAN": ITRMapperService._sanitize(
                                ITRMapperService._txt(v, f"hp_{i}_tenant_pan")
                            ),
                        },
                        "GrossRent": int(rent),
                        "TaxesPaid": int(ITRMapperService._val(v, f"hp_{i}_muni_tax")),
                        "InterestOnBorrowedCapital": int(
                            ITRMapperService._val(v, f"hp_{i}_int_loan")
                        ),
                    }
                )
        return hp_list
    ...
    def _map_schedule_al(v: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "VehiclesBullion": int(
                ITRMapperService._val(v, "al_bullion")
                + ITRMapperService._val(v, "al_vehicles")
            ),
    ...
    def _map_bp(v: Dict[str, Any], form_key: str) -> Dict[str, Any]:
        bp_data = {
            "BusinessProfessionIncome": int(ITRMapperService._val(v, "bp_total"))
        }
        if form_key == "ITR4":
            sec44ad = []
            for i in range(5):
                    ...
                    sec44ad.append(
                        {
                            ...
                            "DigitalTurnover": int(
                                ITRMapperService._val(v, f"ad_{i}_digital")
                            ),
                        }
                    )
            if sec44ad:
            ...
            for i in range(5):
                ...
                if gross > 0:
                    sec44ada.append(
                        {"ProfessionCode": "16019", "GrossReceipts": int(gross)}
                    )
            if sec44ada:
        ...
        for i in range(10):
            ...
            if sale > 0:
                ltcg_112a_list.append(
                    {
                        "ISIN": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"ltcg112a_{i}_isin")
                        ),
                        "NameOfShare": ITRMapperService._txt(v, f"ltcg112a_{i}_name"),
                        "SalePricePerShare": int(sale),
                        "CostOfAcquisition": int(
                            ITRMapperService._val(v, f"ltcg112a_{i}_cost")
                        ),
                        "FMVAsOn31Jan2018": int(
                            ITRMapperService._val(v, f"ltcg112a_{i}_fmv")
                        ),
                        "TotalGain": int(
                            ITRMapperService._val(v, f"ltcg112a_{i}_gain")
                        ),
                    }
                )
        if ltcg_112a_list:
    ...
    def _map_vda(v: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i in range(10):
            ...
            if sale > 0:
                transactions.append(
                    {
                        "DateOfAcquisition": ITRMapperService._txt(
                            v, f"vda_{i}_acq_date"
                        ),
                        "DateOfTransfer": ITRMapperService._txt(
                            v, f"vda_{i}_trans_date"
                        ),
                        "HeadUnderWhichTaxed": "CapitalGains",
                        "CostOfAcquisition": int(
                            ITRMapperService._val(v, f"vda_{i}_cost")
                        ),
                        "ConsiderationReceived": int(sale),
                        "IncomeFromVDA": int(
                            max(0, sale - ITRMapperService._val(v, f"vda_{i}_cost"))
                        ),
                    }
                )
        if transactions:
    ...
    def _map_fa(v: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(5):
            ...
            if value > 0:
                fa_list.append(
                    {
                        "CountryCode": ITRMapperService._sanitize(
                            ITRMapperService._txt(v, f"fa_{i}_country")
                        ),
    ...
    def _map_verification(v: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "Declaration": {
                ...
                "VerificationPAN": ITRMapperService._sanitize(
                    ITRMapperService._txt(
                        v, "verifier_pan", ITRMapperService._txt(v, "pan")
                    )
                ),
