...
class ImportService:
    ...
    def import_trial_balance(form_vars: Dict[str, Any]) -> None:
        ...
        if not path or not ImportService._validate_path(path):
            return
        mapping = {
            "sales": "tb_sales", "purchase": "tb_purchase",
            "direct income": "tb_dir_income", "direct expense": "tb_dir_exp",
            "indirect income": "tb_ind_income", "indirect expense": "tb_ind_exp",
            "salaries": "tb_sal", "rent": "tb_rent",
        }
        imported = 0
        try:
    ...
    def import_prefill(form_vars: Dict[str, Any]) -> None:
        ...
        try:
            ...
            itr_key = next((k for k in ["ITR1", "ITR2", "ITR3", "ITR4"] if k in data.get("ITR", {})), None)
            if not itr_key:
                raise ValueError("Official ITD ITR Schema not found in JSON.")
                
            itr = data["ITR"][itr_key]
            
            p = itr.get("PartA_GEN1", {}).get("PersonalInfo", {}) or itr.get("PersonalInfo", {})
            if "PAN" in p: form_vars["pan"].set(p["PAN"])
            if "AssesseeName" in p and "FirstName" in p["AssesseeName"]: 
                form_vars["name"].set(p["AssesseeName"]["FirstName"])
            if "DOB" in p: form_vars["dob"].set(p["DOB"])
            
            sal = itr.get("PartB_TI", {}).get("Salaries", {}) or itr.get("IncomeDetails", {}).get("Salary", {})
            if "GrossSalary" in sal: form_vars["sal_gross"].set(str(sal["GrossSalary"]))
            if "StandardDeduction" in sal: form_vars["sal_std_ded"].set(str(sal["StandardDeduction"]))
            
            chap6 = itr.get("PartB_TI", {}).get("ChapterVIA", {})
            if "Section80CCD2" in chap6: form_vars["ded_80ccd2"].set(str(chap6["Section80CCD2"]))
            if "Section80JJAA" in chap6: form_vars["ded_80jjaa"].set(str(chap6["Section80JJAA"]))
            
            messagebox.showinfo("Success", f"Prefill data from {itr_key} officially imported and mapped.")
        except Exception as e:
            logger.error(f"Prefill import failed: {e}")
            messagebox.showerror("Import Error", f"Failed to import pre-fill JSON:\n{e}")
    ...
    def import_form16_pdf(form_vars: Dict[str, Any]) -> None:
        ...
        try:
            with open(path, "rb") as f:
                ...
                if tan: form_vars["tds_0_tan"].set(tan.group(0))
                
                gross_sal = re.search(r"Gross Salary.*?(\d{3,})", text, re.IGNORECASE)
                if gross_sal: form_vars["sal_gross"].set(gross_sal.group(1))
                
                messagebox.showinfo("Success", "Form 16 Data (TAN, Salary) seamlessly extracted.")
        except Exception as e:
    ...
    def import_ais_json(form_vars: Dict[str, Any]) -> None:
        ...
        try:
            ...
            total_div = sum(float(item.get("amount", 0)) for item in data.get("dividend_entries", []))
            total_int = sum(float(item.get("amount", 0)) for item in data.get("interest_entries", []))
            ...
            messagebox.showinfo("Success", "AIS data loaded and automatically reconciled to OS schedules.")
        except Exception as e:
    ...
    def extract_with_ai(text: str, root_window=None, on_result: callable = None):
        ...
        Args:
            ...
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle AI results
    ...
    def run_ai_audit(vardict: dict, root_window=None, on_result: callable = None):
        """
        Run AI audit on current VarDict state.
        
        Args:
            vardict: Current application state dict
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle audit results
            
        Returns:
            dict with audit warnings or None if failed
        """
        try:
            from src.services.ai.ai_manager import get_ai_manager
            ...
            ai_manager.audit_vardict_async(
                vardict=vardict,
                callback=handle_result
            )
    ...
    @staticmethod
    def extract_form26as_with_ai(text: str, root_window=None, on_result: callable = None):
        """
        Extract TDS data from Form 26AS text using AI.

        Args:
            text: Raw text from Form 26AS document
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle AI results
    ...
    @staticmethod
    def extract_ais_with_ai(ais_json: str, root_window=None, on_result: callable = None):
        """
        Extract data from AIS JSON using AI.

        Args:
            ais_json: AIS JSON content as string
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle AI results
    ...
    def get_tax_advisory(vardict: dict, root_window=None, on_result: callable = None):
        """
        Get AI-powered tax advisory for current ITR data.

        Args:
            vardict: Current application state dict
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle advisory results

        Returns:
            dict with tax advisory or None if failed
        """
        try:
            from src.services.ai.ai_manager import get_ai_manager
    ...
    def classify_document(content: str, root_window=None, on_result: callable = None):
        ...
        Args:
            ...
            root_window: Root window for .after() callback scheduling
            on_result: Callback function(result_dict) to handle classification result

        Returns:
            dict with document type or None if failed
        """
        try:
            from src.services.ai.ai_manager import get_ai_manager
