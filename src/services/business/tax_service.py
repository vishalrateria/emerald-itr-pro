...
class TaxService:
    ...
    def calculate_gross_total_income(vars_data: Dict[str, Any]) -> float:
        def sf(k):
            return TaxService._get_sf(vars_data, k)
    ...
    def _calculate_house_property(vars_data: Dict[str, Any], sf) -> float:
        ...
        for i in range(MAX_ENTRIES_HP):
            ...
            property_type = (
                property_type.get() if hasattr(property_type, "get") else "let-out"
            )
            ...
            if property_type == "self-occupied":
                ...
                property_net = (
                    nav - (nav * HP_LET_OUT_STD_DEDUCTION_RATE)
                ) - interest_loan
    ...
    def _calculate_business_income(vars_data: Dict[str, Any], sf) -> float:
        ...
        ada_total = sum(
            sf(f"ada_{i}_gross") * PRESUMPTIVE_44ADA_RATE
    ...
    def _calculate_vda(vars_data: Dict[str, Any], sf) -> float:
        return sum(
            max(0.0, sf(f"vda_{i}_sale") - sf(f"vda_{i}_cost"))
    ...
    def calculate_tax_breakdown(vars_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        special_incomes_total = (
            ltcg_112a_total + stcg_sum + ltcg_112 + vda_total + sf("os_winnings")
        )
        ...
        stcg_tax = ITREngine.calculate_stcg_tax(
            stcg_sum, max(0.0, shortfall - (ltcg_112 + ltcg_112a_total))
        )
        vda_tax = ITREngine.calculate_vda_tax(vda_total)
        winnings_tax = ITREngine.calculate_winnings_tax(sf("os_winnings"))
        tax_before_rebate = (
            slab_tax + ltcg_taxes["total"] + stcg_tax + vda_tax + winnings_tax
        )
        ...
        marginal_relief = ITREngine.calculate_marginal_relief_87a(
            tti, slab_tax, is_resident
        )
        ...
        capped_income_total = (
            stcg_sum + ltcg_112a_total + sf("ltcg_112_input") + div_income
        )
