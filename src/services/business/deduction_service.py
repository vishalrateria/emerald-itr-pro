...
class DeductionService:
    ...
    def calculate_allowed_deductions(vars_data: Dict[str, Any]) -> Dict[str, float]:
        sf = lambda k: DeductionService._get_sf(vars_data, k)
        deductions = {
        ...
        if deductions["ded_80ccd2"] > 750000:
            deductions["ded_80ccd2_limited"] = min(deductions.get("ded_80ccd2_limited", deductions["ded_80ccd2"]), 750000)
        total_allowed = sum(v for v in deductions.values() if isinstance(v, (int, float)))
    ...
    def calculate_disallowed_deductions(vars_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        total_disallowed = sum(v for v in disallowed.values() if isinstance(v, (int, float)))
