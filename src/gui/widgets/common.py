...
def page_header(parent, title: str, subtitle: str = "", accent_color: str = None):
    ...
    if subtitle:
        ctk.CTkLabel(
            ...
            anchor="w",
            justify="left",
        ).pack(anchor="w", fill="x", pady=(SPACING_XS, 0))
...
class CollapsibleSection(ctk.CTkFrame):
    def __init__(self, parent, title, is_open=True):
        ...
        self.title_lbl = ctk.CTkLabel(
            self.header,
            text=("▼  " if is_open else "▶  ") + title.upper(),
        ...
        def _on_header_enter(*_args):
            self.header.configure(fg_color=Theme.BG_HOVER)

        def _on_header_leave(*_args):
...
def total_row(
    ...
    lbl = ctk.CTkLabel(
        ...
        text="₹ 0",
        font=Theme.H2,
        text_color=color or Theme.SUCCESS_GREEN,
        anchor="e",
    )
...
def table_header_frame(parent, columns: list):
    ...
    for i, (text, weight) in enumerate(columns):
        ...
        ).grid(row=0, column=i, padx=SPACING_SM, pady=SPACING_SM, sticky="ew")
...
class FluentTable(ctk.CTkFrame):
    def __init__(
        self, parent, title, columns, max_rows, row_generator_fn, validation_refs=None
    ):
        super().__init__(parent, fg_color="transparent")
        ...
        if title:
            ctk.CTkLabel(
                ...
                text_color=Theme.BRAND_BLUE,
                anchor="w",
            ).pack(padx=0, pady=(0, SPACING_SM), anchor="w")
...
def make_card(
    ...
    if accent_color:
        accent = ctk.CTkFrame(
            c, height=2, fg_color=accent_color, corner_radius=Theme.RADIUS_LG
        )
    ...
    if hover:
        ...
        def _on_enter(e):
            c.configure(
                border_color=accent_color if accent_color else Theme.ACCENT_PRIMARY
            )
    ...
    if title:
        ctk.CTkLabel(
            ...
            text=title.upper(),
            font=Theme.H3,
            text_color=title_color or Theme.BRAND_BLUE,
            anchor="w",
        ).pack(padx=INNER_PADX, pady=(SPACING_SM, SPACING_SM), anchor="w")
    return c


def card_spacer(parent):
    ctk.CTkFrame(parent, height=DIVIDER_HEIGHT, fg_color=Theme.SECTION_BORDER).pack(
...
def info_banner(parent, title, text, color=Theme.ACCENT_PRIMARY):
    ...
    ctk.CTkLabel(
        ...
        anchor="w",
        justify="left",
    ).pack(padx=INNER_PADX, pady=(SPACING_SM, SPACING_XS), anchor="w", fill="x")
    ctk.CTkLabel(
        b,
        text=text,
        font=Theme.CAPTION,
        ...
        anchor="w",
        wraplength=600,
    ).pack(fill="x", padx=INNER_PADX, pady=(0, SPACING_SM))
...
def summary_row(
    ...
    row.grid(row=row_idx, column=0, sticky="ew", padx=INNER_PADX, pady=SPACING_SM)
    row.grid_columnconfigure(0, weight=1)
    ...
    if edit_target and switch_fn:
        ctk.CTkButton(
            ...
            font=Theme.CAPTION,
            command=lambda: switch_fn(edit_target),
            **Theme.get_button_style("secondary"),
        ).grid(row=0, column=col, sticky="e", padx=SPACING_XS, pady=SPACING_XS)
        col += 1
    val = ctk.CTkLabel(
        row, text="₹ 0", font=Theme.DATA_BOLD, text_color=Theme.TEXT_PRIMARY, anchor="e"
    )
