...
def init_income_vars(vars_dict, traced_fn, selected_itr_type):
    ...
    for field in [
        ...
        vars_dict[field] = traced_fn(field, "0")
    relief_val = (
        traced_fn("relief_89a", "0")
        if selected_itr_type not in ["ITR-1", "ITR-4"]
        else ctk.StringVar(value="0")
    )
    vars_dict["relief_89a"] = relief_val
    vars_dict["has_89a_relief"] = (
        traced_fn("has_89a_relief", "No")
        if selected_itr_type not in ["ITR-1", "ITR-4"]
        else ctk.StringVar(value="No")
    )
...
def init_business_vars(vars_dict, traced_fn):
    ...
    for field in [
        ...
        "bp_depr",
        "bp_bad_debts",
        "msme_43b_disallowance",
        "msme_interest_disallowance",
        "bp_total_manual",
        "liab_cap_open",
        "liab_cap_close",
        "liab_cap_var",
    ...
    for i in range(MAX_ENTRIES_44AD):
        for f in ["name", "nature"]:
            vars_dict[f"ad_{i}_{f}"] = ctk.StringVar(value="")
        for f in ["turnover", "digital", "inc_44ad"]:
            vars_dict[f"ad_{i}_{f}"] = traced_fn(f"ad_{i}_{f}", "0")
    for i in range(MAX_ENTRIES_44ADA):
        vars_dict[f"ada_{i}_name"] = ctk.StringVar(value="")
        for f in ["gross", "inc_44ada"]:
            vars_dict[f"ada_{i}_{f}"] = traced_fn(f"ada_{i}_{f}", "0")
    for i in range(MAX_BUSINESS_ENTITIES):
        vars_dict[f"gst_{i}_no"] = ctk.StringVar(value="")
        for f in ["turnover", "income", "var"]:
            vars_dict[f"gst_{i}_{f}"] = traced_fn(f"gst_{i}_{f}", "0")
...
def init_bank_vars(vars_dict, traced_fn):
    ...
    for i in range(MAX_ENTRIES_BANK):
        for f in ["ifsc", "acc", "name", "refund"]:
            vars_dict[f"bank_{i}_{f}"] = ctk.StringVar(
                value="" if f != "refund" else "No"
            )


def init_disclosure_vars(vars_dict, traced_fn):
    for f in ["buyback_company", "buyback_acq_date", "buyback_date"]:
        vars_dict[f] = ctk.StringVar(value="")
    for f in ["buyback_shares", "buyback_cost", "buyback_price"]:
...
def init_schedule_vars(vars_dict, traced_fn):
    for i in range(MAX_ENTRIES_TDS):
        for f in ["tan", "name", "section", "cert", "date"]:
            vars_dict[f"tds_{i}_{f}"] = ctk.StringVar(value="")
        vars_dict[f"tds_{i}_amount"] = traced_fn(f"tds_{i}_amount", "0")
    for i in range(MAX_ENTRIES_TCS):
        for f in ["tan", "name", "section", "date"]:
            vars_dict[f"tcs_{i}_{f}"] = ctk.StringVar(value="")
        vars_dict[f"tcs_{i}_amount"] = traced_fn(f"tcs_{i}_amount", "0")
    for i in range(MAX_ENTRIES_TAX_PAID):
        for f in ["bsr", "challan", "date"]:
            vars_dict[f"tax_{i}_{f}"] = ctk.StringVar(value="")
    ...
    for k in [
        ...
        "ltcg_prop_sale",
        "ltcg_prop_cost",
        "ltcg_prop_index_cost",
        "ltcg_prop_relief_amt",
    ]:
    ...
    for i in range(MAX_ENTRIES_AE):
        ...
        for f in ["reg", "tonnage", "months", "amount"]:
            vars_dict[f"ae_{i}_{f}"] = ctk.StringVar(value="")
    for i in range(MAX_ENTRIES_HP):
        for f in [
            ...
            vars_dict[f"hp_{i}_{f}"] = ctk.StringVar(value="")
        vars_dict[f"hp_{i}_muni_tax"] = ctk.StringVar(value="0")
        vars_dict[f"hp_{i}_type"] = ctk.StringVar(value="let-out")
        for f in ["net", "rent", "int_loan", "unrealised_rent"]:
            vars_dict[f"hp_{i}_{f}"] = traced_fn(f"hp_{i}_{f}", "0")
    for i in range(MAX_ENTRIES_VDA):
        for f in ["acq_date", "trans_date", "token", "type", "exchange"]:
            vars_dict[f"vda_{i}_{f}"] = ctk.StringVar(value="")
        for f in ["cost", "sale"]:
            vars_dict[f"vda_{i}_{f}"] = traced_fn(f"vda_{i}_{f}", "0")
    for i in range(MAX_ENTRIES_CG):
        for f in ["isin", "name"]:
            vars_dict[f"ltcg112a_{i}_{f}"] = ctk.StringVar(value="")
        for f in ["sale", "cost", "fmv", "gain"]:
            vars_dict[f"ltcg112a_{i}_{f}"] = traced_fn(f"ltcg112a_{i}_{f}", "0")
    for i in range(MAX_ENTRIES_CG):
        vars_dict[f"stcg_{i}_asset"] = ctk.StringVar(value="")
        for f in ["cost", "sale", "stcg"]:
            vars_dict[f"stcg_{i}_{f}"] = traced_fn(f"stcg_{i}_{f}", "0")
    for i in range(5):
        vars_dict[f"fa_{i}_country"] = vars_dict[f"fa_{i}_nature"] = ctk.StringVar(
            value=""
        )
