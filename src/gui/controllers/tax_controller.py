...
class TaxController:
    ...
    def compute(self) -> bool:
        ...
        excluded_deductions = [
            "ded_80c", "ded_80d", "ded_80e", "ded_80tta", "ded_80ttb",
            "sal_hra", "sal_lta", "ded_80ggc",
            "ded_80g_0_amt", "ded_80g_1_amt", "ded_80g_2_amt", "ded_80g_3_amt", "ded_80g_4_amt"
        ]
        ...
        return True
    def _handle_tax_calculation(self, form_vars: Dict[str, Any], sf: Any, tti: float, vars_dict: Dict[str, Any]) -> Dict[str, float]:
        ...
        form_vars["hp_total"].set(str(int(breakdown.get("gross_total_income", 0) - breakdown.get("normal_income", 0))))
    ...
    def _handle_interest_and_fees(self, form_vars: Dict[str, Any], sf: Any, tti: float, tax_results: Dict[str, float]) -> None:
        ...
        fee_234f = ITREngine.calculate_234f_fee(tti, form_vars["return_filing_date"].get(), self.app.selected_itr_type)
        form_vars["late_fee_234f"].set(str(int(fee_234f)))
        is_revised = (form_vars["is_revised_after_dec31"].get() or "").strip().lower() in ("yes", "1", "true")
        fee_234i = ITREngine.calculate_234i_fee(tti, form_vars["return_filing_date"].get(), is_revised)
        ...
        credits_for_interest = sum(e["amount"] for e in advance_tax_entries) + tds_paid + tcs_paid
        tax_base_for_interest = tax_after_relief
        try:
            advance_buckets = ITREngine.bucket_advance_tax(advance_tax_entries)
            interest_234b = ITREngine.calculate_234b_interest(tax_base_for_interest, advance_buckets, int(sf("months_234b") or 0))
            interest_234c = ITREngine.calculate_234c_interest(tax_base_for_interest, advance_buckets)
            interest_234a = ITREngine.calculate_234a_interest(max(0.0, tax_base_for_interest - credits_for_interest),
                                                              form_vars["return_filing_date"].get(), self.app.selected_itr_type)
            interest_total = interest_234a + interest_234b + interest_234c
        except Exception:
            interest_total = 0.0
        form_vars["interest_total"].set(str(int(interest_total)))
        total_tax_liability = tax_base_for_interest + interest_total + fee_234f + fee_234i
        form_vars["tax_total"].set(str(ITREngine.round_to_nearest_10(total_tax_liability)))
