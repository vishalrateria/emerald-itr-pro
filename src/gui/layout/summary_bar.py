...
class SummaryBarMixin:
    def _setup_summary_bar(self):
        ...
        ctk.CTkFrame(self, height=DIVIDER_HEIGHT, fg_color=Theme.SECTION_BORDER).grid(
            row=1, column=0, sticky="ew", pady=(0, SUMMARY_BAR_HEIGHT - DIVIDER_HEIGHT)
        )
        ...
        for i, (title, key, color) in enumerate(strip_items):
            if i > 0:
                ...
                ).pack(side="left", fill="y", padx=SPACING_SM)
            ...
            ctk.CTkLabel(
                seg, text=title, font=Theme.CAPTION, text_color=Theme.TEXT_MUTED
            ...
            for child in seg.winfo_children():
                ...
                child.bind(
                    "<Enter>", lambda e, s=seg: s.configure(fg_color=Theme.BG_HOVER)
                )
                child.bind(
                    "<Leave>", lambda e, s=seg: s.configure(fg_color="transparent")
                )
        ...
        ).pack(side="right", padx=SPACING_LG)
    ...
    def update_summary_strip(self):
        try:
            ...
            self._strip_labels["strip_gti"].configure(
                text=f"₹ {int(_get_val('gti')):,}"
            )
            self._strip_labels["strip_tti"].configure(
                text=f"₹ {int(_get_val('tti')):,}"
            )
            self._strip_labels["strip_tax"].configure(
                text=f"₹ {int(_get_val('due_tax')):,}"
            )
            net = _get_val("due_tax") - _get_val("it_total")
            self._strip_labels["strip_net"].configure(
                text=f"₹ {int(net):,}",
                text_color=Theme.SUCCESS_GREEN if net <= 0 else Theme.ERROR_RED,
            )
